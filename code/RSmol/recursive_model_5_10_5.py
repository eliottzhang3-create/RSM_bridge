"""SmolLM2 5-10-5 recursive Llama model with explicit logical scheduling.

The 5-10-5 model owns twenty physical decoder modules.  Physical modules 0--4
are the prefix, 5--14 are the recurrent middle, and 15--19 are the suffix.
...
15R implementation remains unchanged.  The cache view translates a physical
layer's ``layer_idx`` to its current *logical* slot, so the two executions of
one middle module use distinct slots (5..14 and 15..24).
"""
...
try:
    ...
    raise ImportError(
        "code.RSmol.recursive_model_5_10_5 requires torch and transformers"
    ) from exc
...
def build_5_10_5_schedule(
    *, logical_layer_count: int = LOGICAL_LAYER_COUNT,
    physical_layer_count: int = PHYSICAL_LAYER_COUNT,
) -> tuple[int, ...]:
    """Validate and return the one true 5-10-5 logical schedule."""

    if int(logical_layer_count) != LOGICAL_LAYER_COUNT:
        raise ValueError(f"5-10-5 requires logical depth 30, got {logical_layer_count}")
    ...
    if len(schedule) != LOGICAL_LAYER_COUNT:
        raise AssertionError("5-10-5 schedule must have exactly 30 logical entries")
    if any(index < 0 or index >= physical_layer_count for index in schedule):
        raise AssertionError("5-10-5 schedule contains an invalid physical index")
    if schedule[:PREFIX_LAYER_COUNT] != tuple(range(5)):
        raise AssertionError("5-10-5 prefix schedule mismatch")
    if schedule[PREFIX_LAYER_COUNT:PREFIX_LAYER_COUNT + MIDDLE_LAYER_COUNT] != tuple(range(5, 15)):
        raise AssertionError("5-10-5 first middle schedule mismatch")
    if schedule[15:25] != tuple(range(5, 15)):
        raise AssertionError("5-10-5 second middle schedule mismatch")
    if schedule[25:] != tuple(range(15, 20)):
        raise AssertionError("5-10-5 suffix schedule mismatch")
    return schedule


def build_5_10_5_source_mapping(num_hidden_layers: int) -> tuple[int, ...]:
    """Return the fixed source mapping after checking source depth is 30."""

    if int(num_hidden_layers) != LOGICAL_LAYER_COUNT:
        raise ValueError(
            "SmolLM2-5-10-5 conversion requires source num_hidden_layers=30; "
...
def logical_slot_for_execution(logical_index: int, physical_index: int) -> int:
    ...
    schedule = build_5_10_5_schedule()
