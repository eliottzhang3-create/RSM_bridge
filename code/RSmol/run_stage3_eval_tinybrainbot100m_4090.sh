...
CMD_PREFIX=""
for name in \
  USER_CONDA_BASE RSMOL_BASELINE_TINYBRAINBOT_MODEL RSMOL_BASELINE_BENCHMARK_ROOT \
  RSMOL_BASELINE_TINYBRAINBOT_OUTPUT_DIR RSMOL_BASELINE_DEVICE RSMOL_BASELINE_DTYPE \
...
vc submit \
  ...
  -c 8 -m 32G -g 1 -n 1 \
  -j stage3-baseline-tinybrainbot100m-4090-$(date +%m%d%H%M%S) \
  -d "$SCRIPT_DIR" \
  JOB=1:1 "$SUBMIT_LOG_ROOT/stage3_baseline_tinybrainbot100m.JOB.log" \
  --cmd "${CMD_PREFIX}bash scripts/evaluate_stage3_baseline_tinybrainbot100m.sh"
