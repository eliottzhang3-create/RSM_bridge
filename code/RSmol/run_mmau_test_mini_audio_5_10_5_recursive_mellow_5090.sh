...
CHECKPOINT="${RSMOL_MMAU_RECURSIVE_CHECKPOINT:-/hpc_stor03/sjtu_home/jinwei.zhang/outputs/RSmol/audio_5_10_5_recursive_mellow/partition_formal_eos_v2_10epochs_20260919/checkpoint-037810}"
...
OUTPUT_DIR="${RSMOL_MMAU_RECURSIVE_OUTPUT_DIR:-/hpc_stor03/sjtu_home/jinwei.zhang/outputs/RSmol/audio_5_10_5_recursive_mellow/mmau_test_mini_checkpoint_037810_strip_label_max32_v2}"
JOB_LOG="$SCRIPT_DIR/log/mmau_audio_recursive_5090.${RUN_TAG}.JOB.log"
...
vc submit \
  ...
  -c 32 -m 256G -g 1 -n 1 \
  -j "mmau-recursive-5090-${RUN_TAG}" \
  -d "$SCRIPT_DIR" \
  JOB=1:1 "$JOB_LOG" \
  --cmd "bash scripts/evaluate_mmau_test_mini_audio_5_10_5_recursive_mellow.sh $CMD_ARGS"

echo "Fixed-recursive MMAU full output directory: ${OUTPUT_DIR}"
