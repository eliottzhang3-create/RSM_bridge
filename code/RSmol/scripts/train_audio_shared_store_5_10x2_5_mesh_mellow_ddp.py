...
from audio_5_10x2_5_mesh_mellow_shared_store.data import ReasonAQADataset, collate_reasonaqa
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    p.add_argument("--model-path", "--mesh-checkpoint", dest="model_path",
                   type=Path, default=Path(base.DEFAULT_MESH))
        ...
        while cursor["global_step"] < stop_step:
            ...
            while (cursor["batch_in_epoch"] < shape["microbatches_per_epoch"]
                ...
                if first_gradient_audit is None:
                    first_gradient_audit = base._mesh_runtime_gradient_audit(owner, require_router_stats=False)
