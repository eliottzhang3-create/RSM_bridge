...
if [[ ! "$EPOCHS_VALUE" =~ ^[1-9][0-9]*$ ]]; then
  echo "configurable shared-store resume2 requires --epochs <positive integer>" >&2
  exit 2
fi
if [[ "$RESUME_SEEN" -ne 1 ]]; then
  echo "resume2 requires --resume-from <matching configurable-epoch checkpoint-000020>" >&2
  exit 2
fi
DEFAULT_OUTPUT="/hpc_stor03/sjtu_home/jinwei.zhang/outputs/RSmol/audio_5_10x2_5_mesh_mellow_shared_store_configurable_epochs/resume2_${EPOCHS_VALUE}epochs_${RUN_ID}"
