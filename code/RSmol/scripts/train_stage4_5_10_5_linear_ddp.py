#!/usr/bin/env python3
"""Stage 4 distributed training for the SmolLM2 5-10-5 linear architecture.
...
DEFAULT_OUTPUT_DIR = Path("/hpc_stor03/sjtu_home/jinwei.zhang/outputs/RSmol/stage4_5_10_5_linear")
...
def _load_runtime_model(model_path: Path, device: Any, tokenizer_path: Path | None = None) -> tuple[Any, Any]:
    ...
    from code.RSmol.recursive_model_5_10_5_linear import register_auto_class
...
def _validate_recursive_architecture(model: Any, *, production: bool = True) -> dict[str, Any]:
    ...
    source_mapping_1based = list(getattr(config, "recursive_source_layer_indices_1based", ()))
    loops_scope = str(getattr(config, "recursive_loops_scope", ""))
    mapping_policy = str(getattr(config, "recursive_mapping_policy", ""))
    ...
    policy_ok = mapping_policy == MAPPING_POLICY or (
        not production and mapping_policy == "stage4_5_10_5_linear_synthetic_fixture"
    )
    if loops != 1 or loops_scope != "none" or (logical, physical) != (20, 20) or schedule != list(LOGICAL_TO_PHYSICAL) or source_mapping != expected_source_mapping or source_mapping_1based != [index + 1 for index in expected_source_mapping] or source_layer_count != 30 or architectures != ["SmolLM2_5_10_5LinearForCausalLM"] or not policy_ok:
        raise ValueError(
            "Stage 4 5-10-5 linear requires logical=20 physical=20 loops=1 and the explicit schedule; "
            f"got logical={logical}, physical={physical}, loops={loops}, loops_scope={loops_scope}, schedule={schedule}, source_mapping={source_mapping}, source_mapping_1based={source_mapping_1based}"
        )
    ...
    layer_parameters = [
        parameter
        for layer in recursive_model.layers
        for parameter in layer.parameters()
    ]
    if len({id(parameter) for parameter in layer_parameters}) != len(layer_parameters):
        raise AssertionError("Linear 5-10-5 decoder layers must not share parameter storage")
    try:
    ...
    try:
        from code.RSmol.recursive_model_5_10_5_linear import parameter_audit
    ...
    return {
        ...
        "decoder_layer_storage_unique": True,
...
def recursive_forward_trace_audit(model: Any, *, device: Any) -> dict[str, Any]:
    """Verify the exact 20-entry linear 5-10-5 physical execution trace."""
    ...
    if sequence != expected:
        raise AssertionError(f"5-10-5 linear forward trace mismatch: expected={expected} got={sequence}")
    return {
        "trace": sequence,
        "expected": expected,
...
def _synthetic_gate_a(model: Any, *, device: Any, config: Stage4Config, rank: int) -> dict[str, Any]:
    """Audit DDP initialization, linear trace, masks, gradients and update."""
    ...
    forward_expected = list(LOGICAL_TO_PHYSICAL)
    backward_expected = list(reversed(LOGICAL_TO_PHYSICAL))
    trace_forward_ok = sequence[:LOGICAL_LAYER_COUNT] == forward_expected
    trace_backward_ok = backward_sequence[:LOGICAL_LAYER_COUNT] == backward_expected
    if not trace_forward_ok:
        raise AssertionError(f"Forward trace is not the 5-10-5 linear schedule: {sequence[:LOGICAL_LAYER_COUNT]}")
    if not trace_backward_ok:
        raise AssertionError(
            "Backward must traverse the 20 linear layers in reverse order: "
    ...
    result = {
        ...
        "forward_trace": sequence[:LOGICAL_LAYER_COUNT],
        "forward_trace_expected": forward_expected,
        "backward_trace": backward_sequence[:LOGICAL_LAYER_COUNT],
        "backward_trace_expected": backward_expected,
        "forward_trace_ok": trace_forward_ok,
        "backward_reverse_order": trace_backward_ok,
...
def _synthetic_model(config: Stage4Config, device: Any) -> Any:
    ...
    from code.RSmol.recursive_model_5_10_5_linear import SmolLM2_5_10_5LinearForCausalLM
    ...
    # complete 20-entry linear 5-10-5 logical schedule.
    logical = 20
    physical = 20
    ...
    llama_config.recursive_loops = 1
    llama_config.recursive_loops_scope = "none"
    llama_config.recursive_layer_count = physical
    llama_config.recursive_source_layer_count = 30
    llama_config.recursive_mapping_policy = "stage4_5_10_5_linear_synthetic_fixture"
    ...
    llama_config.middle_recurrent_count = 0
    ...
    llama_config.architectures = ["SmolLM2_5_10_5LinearForCausalLM"]
    model = SmolLM2_5_10_5LinearForCausalLM(llama_config)
...
def _parse_args(argv: Sequence[str] | None = None) -> Stage4Config:
    ...
    parser.add_argument("--synthetic-layers", type=int, default=20)
    ...
    if config.gate in {"B", "C"}:
        raise ValueError(
            f"Stage 4 5-10-5 linear Gate {config.gate} is unsupported; reuse the existing Gate B audit JSON "
...
def run(config: Stage4Config) -> dict[str, Any]:
    ...
    if str(config.gate).upper() in {"B", "C"}:
        raise ValueError(
            f"Stage 4 5-10-5 linear Gate {config.gate} is unsupported; Gate B/C are intentionally not rerun."
        )
    if config.gate == "D" and config.max_optimizer_steps != DEFAULT_MAX_OPTIMIZER_STEPS:
        raise ValueError(
            "Stage 4 Gate D is a fixed ten-optimizer-step smoke; "
    ...
    try:
        ...
        if config.gate == "B":
            ...
            if model_path is None:
                raise ValueError("Gate B requires --model-path for the converted linear checkpoint")
