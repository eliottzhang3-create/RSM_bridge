#!/usr/bin/env python3
"""Convert a 30-layer SmolLM2 checkpoint to the linear 5-10-5 model.

Only the twenty physical layers in the fixed source mapping are copied. The
destination has 20 logical layers and executes each copied layer once. It is
assembled in a temporary sibling directory and atomically renamed into the
requested external model directory. Existing 15R/recursive files and the Git
checkout are never used as an output location.
"""
...
DEFAULT_OUTPUT_DIR = Path("/hpc_stor03/sjtu_home/jinwei.zhang/models/SmolLM2-5-10-5-linear")
...
def build_target_config(source_config: Any) -> Any:
    """Set the linear 5-10-5 20-logical/20-physical metadata."""

    if int(getattr(source_config, "num_hidden_layers", -1)) != 30:
        raise ValueError("SmolLM2-5-10-5 linear requires source num_hidden_layers=30")
    ...
    target.architectures = ["SmolLM2_5_10_5LinearForCausalLM"]
    return target
...
def build_source_mapping(num_hidden_layers: int = 30) -> tuple[int, ...]:
    ...
    if int(num_hidden_layers) != 30:
        raise ValueError(f"SmolLM2-5-10-5 linear requires source num_hidden_layers=30, got {num_hidden_layers}")
...
def convert(args: argparse.Namespace) -> dict[str, Any]:
    ...
    from recursive_model_5_10_5_linear import SmolLM2_5_10_5LinearForCausalLM, register_auto_class
    ...
    if raw_layers != 30:
        raise ValueError(f"SmolLM2-5-10-5 linear requires source config num_hidden_layers=30, got {raw_layers}")
    ...
    if getattr(source_config, "model_type", None) != "llama":
        raise ValueError("5-10-5 conversion currently supports only Llama/SmolLM2 checkpoints")
    ...
    target_model = SmolLM2_5_10_5LinearForCausalLM(target_config)
    ...
    try:
        ...
        metadata = {
            "status": "ok", "conversion": "SmolLM2-5-10-5-linear", "source_checkpoint": str(source),
            ...
            "source_logical_layer_count": 30, "target_logical_layer_count": 20,
            ...
            "prefix_layer_count": 5, "middle_layer_count": 10,
            "middle_recurrent_count": 0, "suffix_layer_count": 5,
            "loops": 1, "recursive_loops": 1, "loops_scope": "none",
            "recursive_loops_scope": "none", "logical_to_physical": list(LOGICAL_TO_PHYSICAL),
            ...
            "architectures": ["SmolLM2_5_10_5LinearForCausalLM"], "mapping_policy": MAPPING_POLICY,
