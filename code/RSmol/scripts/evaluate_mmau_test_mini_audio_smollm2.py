#!/usr/bin/env python3
"""Run fixed-order MMAU test-mini inference for the audio SmolLM2 baseline.
...
DEFAULT_CHECKPOINT = (
    ...
    "audio_smollm2_135m_mellow/formal_20260911_v1/checkpoint-011343"
)
...
def _load_runtime_model(args: argparse.Namespace) -> tuple[Any, Any, Any, Any]:
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("MMAU audio SmolLM2 inference requires one CUDA GPU")
    ...
    from train_audio_smollm2_135m_mellow_ddp import _audit_saved_checkpoint, _load_model

    config_path = args.checkpoint / "audio_smollm2_config.json"
    if not config_path.is_file():
        raise FileNotFoundError(f"checkpoint audio_smollm2_config.json not found: {config_path}")
...
def _run_model_generation(
    ...
    *,
    ...
    # These helpers build the same 260-token audio prefix and use the same
    # greedy, full-recompute decoder contract as the established comparison.
    from generate_audio_smollm2_checkpoint_reasonaqa import _build_audio_prefix, _greedy_decode
...
def _materialize_from_store(store: ProgressStore, output_dir: Path) -> list[dict[str, Any]]:
    ...
    with (output_dir / "evaluation_predictions.jsonl").open("w", encoding="utf-8") as handle:
        for prediction in predictions:
            handle.write(json.dumps(prediction, ensure_ascii=False, default=_json_default) + "\n")
    first_five = sorted(
...
def _ensure_output_dir(args: argparse.Namespace) -> None:
    ...
    if config_path.is_file():
        ...
        promote_smoke_to_formal = existing_mode == "smoke" and args.mode == "formal"
        mismatches = {
            key: {"existing": existing.get(key), "requested": value}
            for key, value in immutable.items()
            if existing.get(key) != value and not (key == "mode" and promote_smoke_to_formal)
        }
        if mismatches:
            raise RuntimeError(f"existing output directory belongs to another run: {mismatches}")
        if promote_smoke_to_formal:
            # A smoke run intentionally owns the same append-only progress
            # streams that the subsequent formal run can continue.  Promote the
            ...
            if not history or history[-1] != "formal":
                history = [*history, "formal"]
            existing["mode"] = "formal"
            existing["mode_history"] = history
            _write_json(config_path, existing)
        if not promote_smoke_to_formal and existing_mode not in {"", args.mode}:
            # The reverse transition would silently truncate a formal run and is
            # therefore rejected.
            raise RuntimeError(
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    report: dict[str, Any] = {
        ...
        "protocol": {
            ...
            "do_sample": False,
            "use_cache": False,
            "logical_trace": "30 independent standard decoder layers per generation step",
        },
        "records": {},
        "resumption": {},
        "official_evaluation": {},
