...
# The shell hook must be loaded inside the submitted task before activation.
...
MODEL_PATH="${RSMOL_BASELINE_TINYBRAINBOT_MODEL:-/hpc_stor03/sjtu_home/jinwei.zhang/models/Tinybrainbot100M}"
BENCHMARK_ROOT="${RSMOL_BASELINE_BENCHMARK_ROOT:-/hpc_stor03/sjtu_home/jinwei.zhang/data/eval_datasets}"
OUTPUT_DIR="${RSMOL_BASELINE_TINYBRAINBOT_OUTPUT_DIR:-/hpc_stor03/sjtu_home/jinwei.zhang/outputs/RSmol/stage3-baseline-tinybrainbot100m-$(date +%Y%m%d_%H%M%S)-$$}"
DEVICE="${RSMOL_BASELINE_DEVICE:-cuda:0}"
DTYPE="${RSMOL_BASELINE_DTYPE:-float16}"
...
CACHE_DIR="${RSMOL_BASELINE_CACHE_DIR:-/tmp/rsmol-stage3-baseline-tinybrainbot100m-cache-$$}"
