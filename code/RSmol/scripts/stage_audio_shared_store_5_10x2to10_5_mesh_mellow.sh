...
if [[ "$MODE" != "smoke" && "$MODE" != "formal" ]]; then
  echo "invalid variable-depth shared-store mode: $MODE" >&2
  exit 2
fi
...
STAGED_STORE="/dev/shm/rsmol_variable_depth_train_${RUN_ID}"
...
cleanup_shared_store() {
  if [[ -n "${STAGED_STORE:-}" && "$STAGED_STORE" == /dev/shm/rsmol_variable_depth_train_* && -d "$STAGED_STORE" ]]; then
    rm -rf -- "$STAGED_STORE"
  fi
}
...
echo "[variable-depth-stage] copying $SOURCE_STORE to $STAGED_STORE" >&2
...
echo "[variable-depth-stage] PASS store_seconds=$STORE_COPY_SECONDS manifest_seconds=$MANIFEST_COPY_SECONDS total_seconds=$STAGING_TOTAL_SECONDS" >&2

export TORCH_NCCL_ASYNC_ERROR_HANDLING=1
torchrun --standalone --nproc_per_node=8 "$SCRIPT_DIR/train_audio_shared_store_5_10x2to10_5_mesh_mellow_ddp.py" \
  --mode "$MODE" \
