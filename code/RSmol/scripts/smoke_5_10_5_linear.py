#!/usr/bin/env python3
"""Stage 1 audit for a converted SmolLM2 5-10-5 linear checkpoint."""
...
from recursive_model_5_10_5_linear import (  # noqa: E402
    LOGICAL_LAYER_COUNT,
...
def ensure_external_report(path: Path) -> Path:
    ...
    for root in roots:
        try:
            ...
            continue
        raise ValueError(
            "Stage 1 5-10-5 linear smoke refuses to write a report in a Git checkout: "
...
def configured_sample_prompts(primary_prompt: str) -> list[str]:
    ...
    raw = os.environ.get("RSMOL_5_10_5_LINEAR_SAMPLE_PROMPTS", "")
    if not raw.strip():
...
def schedule_trace_audit(model: Any, *, inputs: dict[str, Any]) -> dict[str, Any]:
    ...
    if sequence != expected:
        raise RuntimeError(f"5-10-5 linear forward trace mismatch: expected={expected} got={sequence}")
...
def backward_trace_audit(model: Any, *, inputs: dict[str, Any]) -> dict[str, Any]:
    """Audit reverse schedule and finite gradients for independent layers."""
    ...
    if sequence != expected:
        raise RuntimeError(f"5-10-5 linear backward trace mismatch: expected={expected} got={sequence}")
    shared = {}
    for index in range(PHYSICAL_LAYER_COUNT):
        ...
        if not norms or not all(torch.isfinite(torch.tensor(value)).item() and value > 0 for value in norms):
            raise RuntimeError(f"linear layer {index} has missing/non-finite/zero gradients")
        shared[str(index)] = max(norms)
    return {"trace": sequence, "expected": expected, "loss": float(loss.detach().item()), "layer_gradient_norms": shared, "ok": True}
...
def main() -> None:
    ...
    if device.type != "cuda" or not torch.cuda.is_available():
        raise RuntimeError("5-10-5 linear remote smoke requires a submitted CUDA job")
    device_index = device.index if device.index is not None else torch.cuda.current_device()
    torch.cuda.set_device(device_index)
    if not (model_path / "config.json").is_file():
        raise FileNotFoundError(f"5-10-5 linear checkpoint is missing config.json: {model_path}")

    from transformers import AutoConfig, AutoModelForCausalLM, AutoTokenizer

    print("========== SMOLLM2 5-10-5 LINEAR SMOKE =========", flush=True)
    ...
    print(f"[env] RSMOL_5_10_5_LINEAR_SOURCE_CHECKPOINT={os.environ.get('RSMOL_5_10_5_LINEAR_SOURCE_CHECKPOINT', '<unset>')}", flush=True)
    print(f"[env] RSMOL_5_10_5_LINEAR_MODEL_DIR={os.environ.get('RSMOL_5_10_5_LINEAR_MODEL_DIR', '<unset>')}", flush=True)
    ...
    if (
        ...
        or len(set(int(index) for index in mapping)) != len(mapping)
        or any(int(index) < 0 or int(index) >= source_layer_count for index in mapping)
        or source_layer_count != 30
        or mapping_policy != MAPPING_POLICY
        or list(getattr(config, "logical_to_physical", ())) != list(LOGICAL_TO_PHYSICAL)
        or architectures != ["SmolLM2_5_10_5LinearForCausalLM"]
        or (prefix_count, middle_count, suffix_count) != (5, 10, 5)
        or loops_scope != "none"
    ):
        raise RuntimeError(
            "Invalid 5-10-5 linear config: "
    ...
    expected_schedule = list(build_5_10_5_linear_schedule())
    if list(getattr(config, "logical_to_physical", ())) != expected_schedule:
        raise RuntimeError(
            "5-10-5 linear smoke only accepts the production logical schedule: "
    ...
    print(
        ...
        f"source_layers={source_layer_count} loops={loops} "
    ...
    text = os.environ.get("RSMOL_5_10_5_LINEAR_SMOKE_PROMPT", "Linear 5-10-5 models are")
    ...
    # to select the linear implementation instead of ordinary LlamaForCausalLM.
    register_auto_class()
    ...
    if not isinstance(model, SmolLM2_5_10_5LinearForCausalLM):
    ...
    if len(precreated_cache) < expected_slots:
        raise RuntimeError(
            "Lazy precreated cache did not expand through the linear logical namespace: "
