#!/usr/bin/env python3
"""Offline Stage 3 evaluation for 5-10-5 adjacent-layer average.
...
runtime auditing are isolated to the adjacent-average initialization line.
...
def _load_recursive_5_10_5() -> Any:
    """Load only the adjacent-average runtime without package side effects."""

    return _load_module_from_path(
        "code.RSmol.recursive_model_5_10_5_adjacent_average",
        SCRIPT_ROOT / "recursive_model_5_10_5_adjacent_average.py",
    )
...
MODEL_LABEL = "recursive_5_10_5_adjacent_average"
...
INITIALIZATION_CONTRACT = "prefix_exact_middle_adjacent_pair_fp32_mean_suffix_exact_v1"
...
def _exact_pairs(value: Any) -> tuple[tuple[int, int], ...] | None:
    if not isinstance(value, (list, tuple)):
        return None
...
def inspect_model_artifacts_5_10_5(path: Path) -> dict[str, Any]:
    """Validate the adjacent-average checkpoint before loading weights."""

    model_dir = ensure_external_path(path, label="5-10-5 adjacent-average model")
    ...
    metadata_path = model_dir / "adjacent_average_conversion_metadata.json"
    if not metadata_path.is_file():
        raise FileNotFoundError(
            "Adjacent-average model is missing conversion metadata: "
            f"{metadata_path}"
        )
    conversion_metadata = _read_json(metadata_path)
    metadata_pair_audits = conversion_metadata.get("pair_average_audits")
    metadata_saved_verification = conversion_metadata.get("saved_artifact_verification")
    ...
    initialization_policy = str(config.get("recursive_initialization_policy", ""))
    initialization_contract = str(config.get("recursive_initialization_contract", ""))
    average_dtype = str(config.get("recursive_average_accumulator_dtype", ""))
    prefix_sources = _exact_tuple(config.get("recursive_prefix_source_layers_0based"))
    middle_pairs = _exact_pairs(config.get("recursive_middle_source_layer_pairs_0based"))
    suffix_sources = _exact_tuple(config.get("recursive_suffix_source_layers_0based"))
    source_coverage = _exact_tuple(config.get("recursive_source_layer_coverage_0based"))
    ...
    checks = {
        ...
        "initialization_policy": initialization_policy == INITIALIZATION_POLICY,
        "initialization_contract": initialization_contract == INITIALIZATION_CONTRACT,
        "average_accumulator_dtype": average_dtype == AVERAGE_ACCUMULATOR_DTYPE,
        "prefix_sources": prefix_sources == PREFIX_SOURCE_LAYERS_0BASED,
        "middle_source_pairs": middle_pairs == MIDDLE_SOURCE_LAYER_PAIRS_0BASED,
        "suffix_sources": suffix_sources == SUFFIX_SOURCE_LAYERS_0BASED,
        "source_coverage": source_coverage == SOURCE_LAYER_COVERAGE_0BASED,
        "legacy_source_indices_absent": config.get("recursive_source_layer_indices_0based") is None,
        "legacy_source_mapping_absent": config.get("recursive_source_mapping_0based") is None,
        "metadata_status": conversion_metadata.get("status") == "ok",
        "metadata_initialization_policy": conversion_metadata.get("initialization_policy") == INITIALIZATION_POLICY,
        "metadata_initialization_contract": conversion_metadata.get("initialization_contract") == INITIALIZATION_CONTRACT,
        "metadata_source_pairs": _exact_pairs(conversion_metadata.get("middle_source_layer_pairs_0based")) == MIDDLE_SOURCE_LAYER_PAIRS_0BASED,
        "metadata_source_coverage": _exact_tuple(conversion_metadata.get("source_layer_coverage_0based")) == SOURCE_LAYER_COVERAGE_0BASED,
        "metadata_coverage_exactly_once": conversion_metadata.get("source_layer_coverage_exactly_once") is True,
        "metadata_pair_audit_count": isinstance(metadata_pair_audits, list) and len(metadata_pair_audits) == 10,
        "metadata_pair_audit_sources": isinstance(metadata_pair_audits, list) and tuple(
            tuple(int(item) for item in audit.get("source_pair", ()))
            for audit in metadata_pair_audits
            if isinstance(audit, Mapping)
        ) == MIDDLE_SOURCE_LAYER_PAIRS_0BASED,
        "metadata_pair_audit_targets": isinstance(metadata_pair_audits, list) and tuple(
            int(audit.get("target", -1))
            for audit in metadata_pair_audits
            if isinstance(audit, Mapping)
        ) == tuple(range(5, 15)),
        "metadata_saved_artifact_verification": isinstance(metadata_saved_verification, Mapping) and metadata_saved_verification.get("status") == "PASS",
    ...
    if failed:
        raise ValueError(
            "Invalid SmolLM2-5-10-5 adjacent-average checkpoint contract: "
            f"failed={failed} logical={logical} physical={physical} loops={loops} "
            f"architectures={architectures} initialization_policy={initialization_policy!r} "
    ...
    return {
        ...
        "conversion_metadata_path": str(metadata_path),
        ...
        "recursive_audit": {
            ...
            "initialization_policy": initialization_policy,
            "initialization_contract": initialization_contract,
            "average_accumulator_dtype": average_dtype,
            "prefix_source_layers_0based": list(prefix_sources or ()),
            "middle_source_layer_pairs_0based": [list(pair) for pair in (middle_pairs or ())],
            "suffix_source_layers_0based": list(suffix_sources or ()),
            "source_layer_coverage_0based": list(source_coverage or ()),
...
def recursive_runtime_audit_5_10_5(
    ...
    recursive_module = _load_recursive_5_10_5()
    ...
    if not isinstance(model, RecursiveLlamaForCausalLM):
        raise TypeError(f"AutoModel resolved the wrong 5-10-5 class: {type(model)!r}")
    config = model.config
    base = model.model
    ...
    runtime_initialization = (
        str(getattr(config, "recursive_initialization_policy", "")),
        str(getattr(config, "recursive_initialization_contract", "")),
        str(getattr(config, "recursive_average_accumulator_dtype", "")),
        _exact_tuple(getattr(config, "recursive_prefix_source_layers_0based", ())),
        _exact_pairs(getattr(config, "recursive_middle_source_layer_pairs_0based", ())),
        _exact_tuple(getattr(config, "recursive_suffix_source_layers_0based", ())),
        _exact_tuple(getattr(config, "recursive_source_layer_coverage_0based", ())),
    )
    ...
    if runtime_initialization != expected_initialization:
        raise ValueError(
            "5-10-5 adjacent-average runtime initialization mismatch: "
            f"got={runtime_initialization} expected={expected_initialization}"
        )
    if getattr(config, "recursive_source_layer_indices_0based", None) is not None:
        raise ValueError("Adjacent-average runtime must not advertise legacy source indices")
...
def load_and_audit_recursive_model_5_10_5(
    ...
    try:
        return recursive_runtime_audit_5_10_5(model, tokenizer=tokenizer, device=device)
    finally:
        del model
...
def _run_single_task_5_10_5(
    ...
    try:
        with contextlib.redirect_stderr(captured_stderr):
            ...
            _load_recursive_5_10_5().register_auto_class()
...
def run_evaluation_5_10_5(config: EvaluationConfig) -> dict[str, Any]:
    started_at = utc_now()
    ...
    model_info = inspect_model_artifacts_5_10_5(config.model_path)
    ...
    if not config.validation_only:
        ...
        runtime_audit = load_and_audit_recursive_model_5_10_5(
    ...
    if not config.validation_only:
        for task in config.tasks:
            ...
            try:
                ...
                payload, task_rows = _run_single_task_5_10_5(
                ...
                diagnostic = (
                    f"\n=== Stage 3 5-10-5 adjacent-average task failure: {task} ===\n"
    ...
    if failures:
        raise RuntimeError(
            f"Stage 3 5-10-5 adjacent-average task failures: {sorted(failures)}"
        )
    return audit
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        try:
            ...
            output_was_fresh = False
        audit = run_evaluation_5_10_5(config)
        ...
        if config is not None:
            try:
                ...
                _append_text(
                    ...
                    "\n=== Stage 3 5-10-5 adjacent-average process failure ===\n"
