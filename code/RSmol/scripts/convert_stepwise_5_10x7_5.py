#!/usr/bin/env python3
"""Convert a 30-layer SmolLM2 checkpoint to the shared 5-10-5 model.
...
DEFAULT_OUTPUT_DIR = Path("/hpc_stor03/sjtu_home/jinwei.zhang/models/SmolLM2-5-10-5")
...
def build_target_config(source_config: Any) -> Any:
    """Set logical/physical metadata without changing logical depth."""

    if int(getattr(source_config, "num_hidden_layers", -1)) != 30:
        raise ValueError("SmolLM2-5-10-5 requires source num_hidden_layers=30")
...
def build_source_mapping(num_hidden_layers: int = 30) -> tuple[int, ...]:
    ...
    if int(num_hidden_layers) != 30:
        raise ValueError(f"SmolLM2-5-10-5 requires source num_hidden_layers=30, got {num_hidden_layers}")
...
def convert(args: argparse.Namespace) -> dict[str, Any]:
    ...
    from recursive_model_5_10_5 import RecursiveLlamaForCausalLM, register_auto_class
    ...
    if raw_layers != 30:
        raise ValueError(f"SmolLM2-5-10-5 requires source config num_hidden_layers=30, got {raw_layers}")
    ...
    try:
        ...
        metadata = {
            "status": "ok", "conversion": "SmolLM2-5-10-5", "source_checkpoint": str(source),
            ...
            "source_logical_layer_count": 30, "target_logical_layer_count": 30,
            ...
            "loops": 2, "recursive_loops": 2, "loops_scope": "middle_only",
            "recursive_loops_scope": "middle_only", "logical_to_physical": list(LOGICAL_TO_PHYSICAL),
            ...
            "architectures": ["RecursiveLlamaForCausalLM"], "mapping_policy": MAPPING_POLICY,
