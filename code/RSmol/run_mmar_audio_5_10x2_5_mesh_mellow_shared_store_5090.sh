...
CHECKPOINT="${RSMOL_MMAR_SHARED_STORE_CHECKPOINT:-/hpc_stor03/sjtu_home/jinwei.zhang/outputs/RSmol/audio_5_10x2_5_mesh_mellow_shared_store_configurable_epochs/formal_10epochs_20260923/checkpoint-037810}"
...
OUTPUT_DIR="${RSMOL_MMAR_SHARED_STORE_OUTPUT_DIR:-/hpc_stor03/sjtu_home/jinwei.zhang/outputs/RSmol/audio_5_10x2_5_mesh_mellow_shared_store_configurable_epochs/formal_10epochs_20260923/mmar_checkpoint_037810_verbatim_v1}"
JOB_LOG="$SCRIPT_DIR/log/mmar_shared_store_037810_5090.${RUN_TAG}.JOB.log"
...
vc submit \
  ...
  -c 8 -m 32G -g 1 -n 1 \
  -j "mmar-shared-037810-${RUN_TAG}" \
  -d "$SCRIPT_DIR" \
  JOB=1:1 "$JOB_LOG" \
  --cmd "bash scripts/evaluate_mmar_audio_5_10x2_5_mesh_mellow_shared_store.sh $CMD_ARGS"

echo "Shared-store checkpoint-037810 MMAR full output directory: ${OUTPUT_DIR}"
