...
CMD_PREFIX=""
for name in RSMOL_5_10_5_MODEL_DIR RSMOL_5_10_5_SMOKE_REPORT RSMOL_5_10_5_DEVICE RSMOL_5_10_5_MAX_NEW_TOKENS RSMOL_5_10_5_SMOKE_PROMPT RSMOL_5_10_5_SAMPLE_PROMPTS; do
  value="${!name:-}"
  if [[ -n "$value" ]]; then
...
vc submit \
  ...
  -d "$SCRIPT_DIR" \
  JOB=1:1 "$SUBMIT_LOG" \
  --cmd "${CMD_PREFIX}bash scripts/smoke_recursive_5_10_5.sh"
